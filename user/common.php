...
// PHP dev__ FRAMEWORK # Example $user->get->here(); # output
class common extends site{
	...
	public function admin_top_bar(){   $top_bar = 'hide';
	   if(isset($_REQUEST['devframe_top_bar_hide'])){
		   $_SESSION['devframe_top_bar_hide'] = $_REQUEST['devframe_top_bar_hide']; php_location('');}
		   if(isset($_SESSION['devframe_top_bar_hide'])){
			if($_SESSION['devframe_top_bar_hide']=='hide'){
			echo '<style>#admin_is_login{display:none !important;}</style>';
			$top_bar = 'open';
			}
		}
		return $top_bar;
	}
	public function admin_session_data(){ # DEFAULT FAILD ADMIN
		    mysql_query("CREATE TABLE IF NOT EXISTS ".DATABASE.".`".DB_PREFIX."session_data` (
		...
		) ENGINE = InnoDB"); # Create table is not exits			
	}
	...
	public function admin_access_count(){ 
	   $slt =  mysql_query("select count(*) from ".DB_PREFIX."session_data");
	   return mysql_result($slt,0);
	}
