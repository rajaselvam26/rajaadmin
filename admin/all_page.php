...
if(isset($_REQUEST['type'])){
	...
	$res = sub_pagination(DB_PREFIX."posts where extension_file='' and plugin_file=''",10,"Posts&Admin_page=Posts&type=post");
	$count = num(DB_PREFIX."posts where extension_file='' and plugin_file=''");
	$in='Posts'; $idname='p_id';
	$pageCall = 'Editpost'; 	
	}else{
	$res = sub_pagination(DB_PREFIX."pages where extension_file='' and plugin_file=''",10,"Pages&Admin_page=Pages&type=page");
	$count = num(DB_PREFIX."pages where extension_file='' and plugin_file=''");
...
<td><a href="Javascript:Go('?<?php echo $in;?>&Admin_page=<?php echo $pageCall;?>&<?php echo $idname;?>=<?php echo ($_REQUEST['type']=='post')?$page->post_id:$page->page_id;?>');">Edit</a>&nbsp;<a onclick="return dev_delete('delete_load.php','ajax_view','<?php echo ($_REQUEST['type']=='post')?$page->post_id:$page->page_id;?>','<?php echo $in;?>');" class="oncl" >Delete</a></td>
