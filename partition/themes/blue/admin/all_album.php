...
xmlhttp.open("GET","film.php?q="+str,true);
xmlhttp.send();
}	
</script>
<?php
if(isset($_REQUEST['sub']))
{
	...
	mysql_query("update flim_english set flag='".$type."' where id='".$filmid."'");
	mysql_query("update flim_tamil set flag='".$type."' where id='".$filmid."'");
	if($up!='')
	{
		move_uploaded_file($_FILES['up']['tmp_name'],"../Thumbnails/".$up);
		mysql_query("update flim_english set image='".$up."' where id='".$filmid."'");
	mysql_query("update flim_tamil set image='".$up."' where id='".$filmid."'");
	}
	header("location:?Theme_admin=all_album&Albums Management");
}
?>
<?php
if(isset($_REQUEST['del_id'])){
$del=mysql_query("update flim_english set flag=0 where id='".$_REQUEST['del_id']."'");
mysql_query("update flim_tamil set flag=0 where id='".$_REQUEST['del_id']."'");

echo "The Album  is canceled";
}
...
<th width="32">Type</th>
<th width="60">Uploaded Thumbnail</th>
<th width="32">Cancel</th>
<th width="32">Edit</th>
<!--<th width="60">Edit</th>-->
</tr>
...
$vtb=mysql_query("select * from  flim_english where flag='1' or flag='2'  order by flag desc");
...
<td><?php if($r->flag==1){echo "Private Album";}else if($r->flag==2){echo "Latest Tracks";} ?></td>
<?php if($r->image!=''){ ?>
<td><img src="../Thumbnails/<?php echo $r->image; ?>"  width="122px" height="137px"/></td><?php }else{?>
<td></td><?php }?>
<?php /*?><?php if($r->page==1){ ?>
    <td>Home Page(Top Right)</td><?php } else if($r->page==2){?><td>Home Page (Bottom)</td><?php }?><?php */?>
<td><a href="javascript:void('0');" onclick="return delete_me('<?php echo $r->id?>');">Cancel</a></td>
<td align='left'><a href='?Theme_admin=all_album&Albums Management&id=<?php echo $r->id?>&n=1'>Edit</a></td></tr>
<?php } ?>
<?php //} ?>
</tr>
</table></form>
...
{
	?>
	<div id="apDiv1" style="width:360px  !important; "><div align="right"><a href="?Theme_admin=all_album&Albums Management">Close</a></div>
    <link rel="stylesheet" href="http://code.jquery.com/ui/1.10.2/themes/smoothness/jquery-ui.css" />
  <script src="http://code.jquery.com/jquery-1.9.1.js"></script>
  <script src="http://code.jquery.com/ui/1.10.2/jquery-ui.js"></script>
  <form enctype="multipart/form-data" method="post" role="form">
<table width="364" class="dash_count_table" cellspacing="0">
<tr class="tr_th">
<!--<th align="center">Select Page</th>-->

          <td width="100">Film Name</td>
        <td align="left"><div  class="ui-widget">
  <input id="suggestionbox" name="fname" placeholder="Film Name" value="<?php echo $r1->flimname; ?>" style="border-radius:5px;" onblur="shwfilm(this.value);" required="required"  />
</div></td></tr><tr class="tr_th">
         <td width="100">Film Id</td>
        <td><div id="rep"><input type="text"   name="filmid" value="<?php echo $r1->id; ?>" readonly="readonly"/>
                                    ...
                                    <option value="1">Private Album</option>
                                    <option value="2">Latest Tracks</option>
                                    </select></td></tr><tr class="tr_th">
