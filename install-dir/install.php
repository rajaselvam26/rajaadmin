...
new DB;");
	...
	if(in_array($_REQUEST['db'],$db)){ $sql = 'DROP DATABASE '.$_REQUEST['db'].''; mysql_query($sql,$con);} # delete exist database from server
	  ...
	  mysql_select_db($_REQUEST['db'],$con);
		...
		mysql_query("CREATE TABLE ".$_REQUEST['db'].".`".$_REQUEST['db_prefix']."extensions` (
		...
		mysql_query("CREATE TABLE ".$_REQUEST['db'].".`".$_REQUEST['db_prefix']."session_data` (
		...
		if($Host==true){
			...
			mysql_query("insert into ".$_REQUEST['db_prefix']."themes(theme,status) values ('blue','1')");			
		if($Insert==true){
...
<div id="wrapper">
<div id="header">
<strong>Install dev__ framework</strong>
</div>
