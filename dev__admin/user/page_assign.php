...
class page_assign extends left_menu{
	public function admin_page_assign(){ // Assign Page
		...
		switch($page){
			...
			case 'Plugins':require 'all_plugin.php';break; //Plugins .php
			case 'Addplugin':require 'add_plugin.php';break; 
			case 'Extension':require 'all_extension.php';break; //Extension
			case 'Addextension':require 'add_extension.php';break; 
			case 'themes':require 'all_theme.php';break; //Themes
			case 'Addtheme':require 'add_theme.php';break; 
			case 'trash_themes':require 'trash_theme.php';break; //Trash Themes
			case 'trash_plugins':require 'trash_plugin.php';break; //Trash Plugins
   ...
   public function admin_page_ext_integration(){ // From Plugin Path
	    $plugin_admin_page = 'false';
		$plug_admin_path = array();
		foreach(glob('../partition/extensions/*',GLOB_ONLYDIR) as $plug_folder){   
			   $plug_admin_path[]=$plug_folder;
		}
		$i=-1; $plugin_i = -1;
		foreach($plug_admin_path as $admin){ $plugin_i++;
			   ...
			   foreach(glob($admin.'/*',GLOB_ONLYDIR) as $left_admin){ $i++;
					if(basename($left_admin)=='admin'){
						if(file_exists($left_admin.'/index.php')){
						require_once $left_admin.'/index.php';
						if(num(DB_PREFIX."extensions where extension='".$plugin."' and status=1")==1){
						if(class_exists($GLOBALS['extension_admin'][$plugin_i])){
						
							$obj = new $GLOBALS['extension_admin'][$plugin_i];
							if(method_exists($GLOBALS['extension_admin'][$plugin_i],'page')){
								if(isset($_REQUEST[$GLOBALS['extension_admin'][$plugin_i]])){
									$obj->page(); 
									$plugin_admin_page='true';
								}
							}
